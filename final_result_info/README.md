# Final result info — gói dữ liệu cho AI nội bộ

Đây là bản sao **information-only** của:

`H:\My Drive\GAN\GAN\final_result`

Mục tiêu là giữ đầy đủ dữ liệu văn bản/báo cáo để AI phân tích mà không mang
theo model, checkpoint hoặc serialized estimator.

## Chính sách lọc

Được giữ:

- CSV/TSV: payload, feature, epoch metrics, run matrix, bảng chỉ mục;
- JSON/JSONL: manifest, training metadata, quality, RF, tokenizer, summary;
- Markdown/TXT/LOG: README, giải thích, log huấn luyện và log đánh giá;
- XLS/XLSX/YAML/YML nếu có.
...
Notebook bị loại vì là source code, không phải report. Nội dung phương pháp,
quyết định và phân tích đã được giữ trong README bên dưới và các artifact
JSON/CSV.

## Tính đầy đủ của thông tin

- 489 run manifest;
- 489 training metadata;
- 489 quality reports;
- 489 RF reports;
- 489 generated payload CSV;
- 489 generated feature CSV;
- JSON parse errors: 0;
- `.pt`/`.joblib`/`.ipynb` còn lại trong gói: 0.

Manifest lọc:

- [`_info_index/included_files.csv`](_info_index/included_files.csv)
- [`_info_index/excluded_files.csv`](_info_index/excluded_files.csv)
- [`_info_index/summary.json`](_info_index/summary.json)

README phân tích nguyên bản của full result được bảo tồn riêng tại
[`_provenance/README_FULL_RESULT.md`](_provenance/README_FULL_RESULT.md).

---

## Phân tích khoa học của full result

Phần dưới là nội dung README phân tích của bộ full result. Những đoạn nói về
`.pt`/checkpoint mô tả bộ nguồn; các binary đó không hiện diện trong gói
information-only này.
